...
const validate = () => {
  ...
  if (!utils.configHas(requiredFields)) {
    logger.error()
    logger.error(`You must set these variables in ${context.get('path.config')} to setup Elastic Cloud:`)
    logger.error()
...
module.exports = async () => {
  ...
  if (state.get('plugins.elastic-cloud.deployment_id')) {
    logger.info('')
    logger.info(`Deployment ID exists in .state file: ${state.get('plugins.elastic-cloud.deployment_id')}`)
    logger.info('')
    logger.info('Checking if the deployment exists on Elastic Cloud...')
    deploymentExists = await probe.statusElasticCloud()
    if (deploymentExists)
      logger.info(`...deployment exists on Elastic Cloud: 'microbs-${config.get('deployment.name')}' [deployment_id=${state.get('plugins.elastic-cloud.deployment_id')}]`)
    else
  ...
  if (!deploymentExists) {
    ...
    state.set('plugins.elastic-cloud.deployment_id', response.data.id)
    state.save()
    ...
    for (var i in response.data.resources) {
      ...
      if (resource.kind == 'elasticsearch') {
        state.set('plugins.elastic-cloud.cloud_id',  _.get(resource, 'cloud_id'))
        state.set('plugins.elastic-cloud.elasticsearch.username', _.get(resource, 'credentials.username'))
        state.set('plugins.elastic-cloud.elasticsearch.password', _.get(resource, 'credentials.password'))
      }
      if (resource.kind == 'integrations_server') {
        state.set('plugins.elastic-cloud.integrations_server.secret_token', _.get(resource, 'secret_token'))
      }
    }
    state.save()
    ...
    for (var i in components) {
      ...
      while (!found) {
        try {
          var response = await utils.http({
            method: 'get',
            url: `https://api.elastic-cloud.com/api/v1/deployments/${state.get('plugins.elastic-cloud.deployment_id')}/${component}/main-${component}`,
            headers: constants.elasticCloudApiHeaders()
          })
          var url = _.get(response.data, 'info.metadata.service_url')
          if (url) {
            if (component == 'elasticsearch') {
              state.set('plugins.elastic-cloud.elasticsearch.url', url)
            } else if (component == 'kibana') {
              state.set('plugins.elastic-cloud.kibana.url', url)
            } else if (component == 'integrations_server') {
              state.set('plugins.elastic-cloud.integrations_server.url', url)
              state.set('plugins.elastic-cloud.integrations_server.exporter_endpoint', `${(new URL(url)).hostname}:443`)
            }
    ...
    logger.info('')
    logger.info(`Kibana URL:              ${state.get('plugins.elastic-cloud.kibana.url')}`)
    logger.info(`Elasticsearch URL:       ${state.get('plugins.elastic-cloud.elasticsearch.url')}`)
    logger.info(`  - Username:            ${state.get('plugins.elastic-cloud.elasticsearch.username')}`)
    logger.info(`  - Password:            ${state.get('plugins.elastic-cloud.elasticsearch.password')}`)
    logger.info(`Integration Server URL:  ${state.get('plugins.elastic-cloud.integrations_server.url')}`)
  }
